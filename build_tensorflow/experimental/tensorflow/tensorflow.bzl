...
def tf_cc_test_mkl(
    ...
    for src in srcs:
        native.cc_test(
            ...
            copts = tf_copts(),
            linkopts = ["-lpthread", "-lm"] + _rpath_linkopts(src_to_test_name(src)),
...
def tf_cuda_library(deps = None, cuda_deps = None, copts = tf_copts(), **kwargs):
    ...
    native.cc_library(
        ...
        copts = (copts + if_cuda(["-DGOOGLE_CUDA=1"]) + if_mkl(["-DINTEL_MKL=1"]) + if_mkl_open_source_only(["-DINTEL_MKL_DNN_ONLY"]) + if_enable_mkl(["-DENABLE_MKL"]) + if_tensorrt(["-DGOOGLE_TENSORRT=1"])),
        **kwargs
    )
