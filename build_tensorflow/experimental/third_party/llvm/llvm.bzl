...
def expand_cmake_vars(name, src, dst, cmake_vars):
    ...
    Args:
      ...
      cmake_vars: a string containing the CMake variables, as generated by
        cmake_var_string.
    """
    expand_cmake_vars_tool = Label("@tfx_experimental//third_party/llvm:expand_cmake_vars")
    native.genrule(
        name = name,
        srcs = [src],
...
llvm_all_cmake_vars = select({
    "@tfx_experimental//tensorflow:darwin": cmake_var_string(
        _dict_add(
            cmake_vars,
    ...
    "@tfx_experimental//tensorflow:linux_ppc64le": cmake_var_string(
        _dict_add(
            cmake_vars,
    ...
    "@tfx_experimental//tensorflow:windows": cmake_var_string(
        _dict_add(
            cmake_vars,
...
def llvm_support_platform_specific_srcs_glob():
    return select({
        "@tfx_experimental//tensorflow:windows": native.glob([
